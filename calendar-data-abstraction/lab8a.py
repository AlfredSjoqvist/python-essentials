...
def ts_overlapping_part(ts1: TimeSpan, ts2: TimeSpan) -> TimeSpan:
    ...
    #  Added by Daniel
    ts_1_start_min = hour_number(time_hour(ts_start(ts1))) * 60 + minute_number(time_minute(ts_start(ts1)))
    ts_1_end_min = hour_number(time_hour(ts_end(ts1))) * 60 + minute_number(time_minute(ts_end(ts1)))

    ts_2_start_min = hour_number(time_hour(ts_start(ts2))) * 60 + minute_number(time_minute(ts_start(ts2)))
    ts_2_end_min = hour_number(time_hour(ts_end(ts2))) * 60 + minute_number(time_minute(ts_end(ts2)))
    
    min1 = max(ts_1_start_min, ts_2_start_min)
    min2 = min(ts_1_end_min, ts_2_end_min)
...
def ts_duration(ts: TimeSpan) -> "Duration":
    ...
    mins = (time_span_end - time_span_beggining)
...
def duration_is_longer_or_equal(d1: Duration, d2: Duration):
    ...
    mins2 = minute_number(duration_minute(d1))
